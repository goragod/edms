...
class View extends \Gcms\View
{
    ...
    public function render($index, $login)
    {
        ...
        $content .= '<p class=tr><span class="td item icon-edit">{LNG_Detail}</span><span class="td item center">&nbsp;:&nbsp;</span><span class="td item">'.nl2br($index->detail).'</span></p>';
        if ($index->url != '') {
            $content .= '<p class=tr><span class="td item icon-world">{LNG_URL}</span><span class="td item">:</span><span class="td item"><a href="'.$index->url.'" target=_blank>'.$index->url.'</a></span></p>';
        }
        $content .= '</div>';
        if ($index->url == '') {
